...
- `ID` は正の整数。CLI は機器固有の上限や対象の存在までは検証しないため、実機で照合する。
- その位置で `config wlan disable ID` を実行し、次のサイクルの直前または処理の最後で
  `config wlan enable ID` を実行する。コマンドとサイクルの引数順が実行順になる。
- 元の有効・無効状態は記録しない。元から無効の WLAN を無効のまま保つ場合には使わず、
  その状態に合う変更手順を組む。
- 例外時も、無効化を試みた WLAN の再有効化を試みる。復旧は保証されず、投入済みの設定を
  元に戻す処理もない。無効化・再有効化のタイムアウト時は次のサイクルへ進まない。

## 結果を確認し、必要なら保存する

終了コードと機器の出力の両方を確認し、変更後の表示コマンドで設定値と WLAN の状態を検証する。
プロンプトに戻ったことだけを成功と扱わない。CLI は構文エラーなどの機器側メッセージを
自動判定しないため、終了コード 0 でも変更に失敗している場合がある。
...
`--save` は `save config` を送る。変更と同時に付けることもでき、その場合は全 WLAN の
再有効化後に保存する。ただし機器側エラーを自動判定しないため、通常は検証後の別実行にする。
保存処理も出力を確認し、確認できなければ「保存済み」と断定しない。

通常コマンドは120秒無出力になると次のコマンドへ進み、最終的に終了コード 1 となり
`--save` を実行しない。失敗後の依存コマンドを避けたい場合は呼び出しを分ける。
