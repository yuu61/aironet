...
- 例外時も元の状態への復旧を試みる。タイムアウト時は `q` (`--More--` 待ち) または Ctrl-Z で
  ルートプロンプトへ戻ることを試み、戻れば復旧を行う。切断やプロンプトへ戻れない場合は
  復旧できず、エラーを表示する。
  投入済みの設定値を元に戻す処理はない。失敗時は次のサイクルへ進まない。
...
確認待ちの行 (行末が `(y/n)`。前置きの警告文が同じ行に付く形も含む) に `y` を返し、
Enter 待ちには Enter、`--More--` には Space を返す。
大量出力による切断を避けるため、接続後はページ送りを有効にする。`config paging` は
read-write 権限が要るため、read-only ユーザーでは警告して続行する (表示コマンドは使える)。
既知の機器側エラー、状態確認の失敗、120秒の無出力で処理を停止し、終了コード 1 となる。
後続コマンドと `--save` を実行しない。タイムアウト後は `q` / Ctrl-Z でプロンプトへ戻れた場合だけ
WLAN の復旧を行い、戻れなければ同じ接続へ追加のコマンドを送らない。
